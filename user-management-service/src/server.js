...
const passportJWT = require('./config/passport')(passport); // Change this path to match your passport.js location
...
var server = app.listen(process.env.UMS_APP_PORT || 1001, function () {
    ...
    logger.info(`[${fn}]: The user-management-service has started on port ${server.address().port} in ${process.env.UMS_NODE_ENV} mode.`, { filename: fn, port: server.address().port, mode: process.env.NODE_ENV });
