...
const servicePort = process.env.UMS_API_PORT;
const serviceKey = process.env.UMS_API_KEY;

// Build the URL for the UMS API
const connString = `http://${serviceHost}:${servicePort}/users/`;
...
const fetchUser = async (userId) => {
    try {
        ...
        if (error.response && error.response.status === 404) {
            ...
            logger.error(`[${fn}]: Error checking if user ${userId} exists on the user-management-service.`, { error: error })
